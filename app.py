...
if st.session_state.records:
    ...
    with col1:
        if st.button("🗑️ 清除預覽清單", use_container_width=True):
            st.session_state.records = []  # 清除預覽資料
            st.session_state.reset_key += 1  # 改變 key 以清空上方日期選擇欄位
            st.rerun() 
            ...
            if name == "請選擇":
                ...
                with st.spinner('提交中...'):
                    ...
                    if count == len(st.session_state.records):
                        ...
                        st.session_state.records = []
                        st.session_state.reset_key += 1 # 提交成功也清空欄位
                        st.balloons()
                        st.rerun()
                    elif count > 0:
